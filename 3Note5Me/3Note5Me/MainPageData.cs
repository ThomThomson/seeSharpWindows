    ...
    class MainPageData : INotifyPropertyChanged{
        ...
        private async Task PopulateNotes() {
            ...
            foreach (StorageFile currentFile in files) {
                Note noteToAdd = new Model.Note(id, currentFile.Name.Substring(0, currentFile.Name.LastIndexOf(".")), await FileIO.ReadTextAsync(currentFile));
                noteToAdd.File = currentFile;
                Notes.Add(noteToAdd);
                id++;
            }
        }//E N D  M E T H O D PopulateNotes
